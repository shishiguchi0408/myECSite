...
public class MyPageDAO {
	...
	public ArrayList<MyPageDTO> getMyPageUserInfo//DBから購入履歴を取得するためのメソッド。
		...
		String sql = "SELECT ubit.id,iit.item_name,ubit.total_price,ubit.total_count,ubit.pay,ubit.insert_date "
					...
					+ "LEFT JOIN item_info_transaction iit "//LEFT JOINで複数のテーブルを結合し、ユーザー情報と履歴情報を紐付けて一括して取得。
		...
		try{
			...
			while(resultSet.next()){
				...
				dto.setId(resultSet.getString("id"));//取得した結果を一件ずつMyPageDTOに格納し、
				dto.setItemName(resultSet.getString("item_name"));
				dto.setTotalPrice(resultSet.getString("total_price"));
				dto.setTotalCount(resultSet.getString("total_count"));
				dto.setPayment(resultSet.getString("pay"));
				dto.setInsert_date(resultSet.getString("insert_date"));
				myPageDTO.add(dto);//更に、ArrayListのmyPageDTOに格納している。
			}
		}catch(Exception e){
			e.printStackTrace();
		}finally{
	...
	//DBから購入履歴を削除するためのメソッド。(MyPageActionのdeleteFlgが1になったら飛んでくる。)
	public int buyItemHistoryDelete(String item_transaction_id,String user_master_id) throws SQLException{
		...
		return result;//MyPageActionに削除した件数を返している。
	}
